...
export function getByCategory(inventory, category) {
  return inventory.filter(
    (item) => String(item.cat || "").toLowerCase() === category.toLowerCase()
  );
}
...
export function sortByQty(inventory) {
  return [...inventory].sort((a, b) => Number(a.qty || 0) - Number(b.qty || 0));
}
...
export function getMemberBreakdown(expenses) {
  const breakdown = {};
  expenses.forEach((e) => {
    const member = e.member || "Other";
    breakdown[member] = (breakdown[member] || 0) + Number(e.amount || 0);
  });
  return breakdown;
}
...
export function sortExpensesByAmount(expenses) {
  return [...expenses].sort((a, b) => Number(b.amount || 0) - Number(a.amount || 0));
}

// Sort expenses by date newest first
export function sortExpensesByDate(expenses) {
  return [...expenses].sort((a, b) => Number(b.date || 0) - Number(a.date || 0));
}
